...
def get_stats():
    ...
    batters_dict = pd.read_csv('Pitchers_Dict.csv')
...
def compile_stats():
    ...
    df_stats_dict['Power_Score'] = (df_stats_dict['H_Rank']+df_stats_dict['R_Rank']+df_stats_dict['HR_Rank']+df_stats_dict['RBI_Rank']+df_stats_dict['SB_Rank']+df_stats_dict['OPS_Rank'])/6
    ...
    print(df_join)
    #Rank after join
    df_join = df_join.dropna(subset=['Team','Pick'])
    df_join['Power_Rank'] = df_join['Power_Score'].rank(ascending = True)
    df_join['Pick_Power'] = df_join['Overall_Pick_x'] - df_join['Power_Rank']
    df_join['Pick_Rank'] = df_join['Pick_Power'].rank(ascending = False)
